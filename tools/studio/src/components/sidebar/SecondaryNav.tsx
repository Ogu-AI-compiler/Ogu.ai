...
const ITEMS = [
  ...
  { key: "/audit", label: "Audit", icon: icons.shield },
  { key: "/marketplace", label: "Marketplace", icon: icons.marketplace },
  { key: "/settings", label: "Settings", icon: icons.settings },
  { key: "/kadima", label: "Kadima", icon: icons.terminal },
];
      ...
      {ITEMS.map((item) => {
        ...
        return (
          <button
            ...
            className="flex items-center gap-3 px-3 py-1.5 rounded-lg text-[12px] font-medium cursor-pointer"
            style={{
              background: "transparent",
              border: "none",
              color: isActive ? "var(--color-text)" : "rgba(255,255,255,0.35)",
