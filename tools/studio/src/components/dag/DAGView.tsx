...
export function DAGView() {
  ...
  if (loading) return <Container><Text color="$colorPress">Loading DAG...</Text></Container>;

  if (error || !dag) {
    return (
      <Container>
        <Card>
          <Text fontSize="$3" fontWeight="600">Task Dependency Graph</Text>
          <Text fontSize="$2" color="$colorPress">
            {error || "No DAG data available."}
          </Text>
          {activeFeature && (
            <ActionButton
  ...
  return (
    <Container>
      ...
      <XStack justifyContent="space-between" alignItems="center">
        <Text fontSize="$5" fontWeight="700">Task DAG — {activeFeature}</Text>
        <ActionButton
          label="Regenerate"
          variant="ghost"
          onAction={handleGenerate}
      ...
      <XStack gap="$3" flexWrap="wrap">
        <StatBox>
          <Text fontSize={10} color="$colorPress">Tasks</Text>
          <Text fontSize="$4" fontWeight="700">{dag.total_tasks}</Text>
        </StatBox>
        <StatBox>
          <Text fontSize={10} color="$colorPress">Waves</Text>
          <Text fontSize="$4" fontWeight="700">{waves.length}</Text>
        </StatBox>
        <StatBox>
          <Text fontSize={10} color="$colorPress">Max Parallel</Text>
          <Text fontSize="$4" fontWeight="700">{dag.max_parallelism}</Text>
        </StatBox>
        <StatBox>
          <Text fontSize={10} color="$colorPress">Conflicts</Text>
          <Text fontSize="$4" fontWeight="700" color={conflicts.length > 0 ? "#facc15" : "#4ade80"}>
            {conflicts.length}
          </Text>
        </StatBox>
        {dag.critical_path?.length > 0 && (
          <StatBox>
            <Text fontSize={10} color="$colorPress">Critical Path</Text>
            <Text fontSize="$4" fontWeight="700" color="#a78bfa">{dag.critical_path.length} tasks</Text>
          </StatBox>
        )}
      </XStack>
      ...
      <XStack gap="$3" flexWrap="wrap" alignItems="center">
        {Object.entries(STATUS_COLORS).filter(([k]) => ["pending", "running", "completed", "failed"].includes(k)).map(([status, color]) => (
          <XStack key={status} gap="$1" alignItems="center">
            <div style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: color }} />
            <Text fontSize={10} color="$colorPress">{status}</Text>
          </XStack>
        ))}
        <Separator alignSelf="stretch" vertical borderColor="rgba(255,255,255,0.1)" />
        <XStack gap="$1" alignItems="center">
          <div style={{ width: 16, height: 2, backgroundColor: "#a78bfa", borderRadius: 1 }} />
          <Text fontSize={10} color="$colorPress">critical path</Text>
        </XStack>
        <XStack gap="$1" alignItems="center">
          <div style={{ width: 16, height: 2, backgroundColor: "#ef4444", borderRadius: 1, borderTop: "1px dashed #ef4444" }} />
          <Text fontSize={10} color="$colorPress">conflict</Text>
        </XStack>
      </XStack>
          ...
          >
            ...
            {waves.map((wave: any, i: number) => (
              <WaveLabel
                key={i}
                waveIndex={i}
                x={PADDING + i * WAVE_GAP}
                taskCount={(wave.tasks || []).length}
                parallel={wave.parallel !== false}
              />
            ))}
      ...
      {conflicts.length > 0 && (
        <Card>
          <Text fontSize="$3" fontWeight="600" color="#facc15">Conflicts ({conflicts.length})</Text>
          <YStack gap="$2">
            {conflicts.map((c: any, i: number) => (
              <XStack key={i} backgroundColor="rgba(250,204,21,0.05)" borderRadius="$1" padding="$2" gap="$2">
                <Text fontSize={10} color="#facc15" fontWeight="600">
                ...
                <Text fontSize={10} color="$colorPress">{c.resolution || "sequential"}</Text>
              </XStack>
            ))}
          </YStack>
        </Card>
