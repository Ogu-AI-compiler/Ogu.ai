...
function SourceCard({ source, onRemove }: { source: ProjectSource; onRemove: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onRemove}
        className="shrink-0 p-1 rounded-md text-text-muted hover:text-text hover:bg-bg-elevated transition-colors cursor-pointer"
      >
    ...
    <div className="flex flex-col gap-2 rounded-xl border border-border bg-bg-card overflow-hidden">
      ...
      <div ref={scrollRef} className="flex flex-col max-h-[240px] overflow-y-auto px-1 pb-1">
        ...
        {currentPath !== "/" && (
          <button
            onClick={goUp}
            className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-text-muted hover:bg-bg-elevated hover:text-text transition-colors cursor-pointer text-left"
          >
  ...
  const runScan = async () => {
    ...
    try {
      ...
      const [profileRes, repoMapRes] = await Promise.all([
        fetch("/api/command/sync", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ command: "profile" }),
        }).then((r) => r.json()).catch(() => ({ stdout: "" })),
        fetch("/api/command/sync", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ command: "repo-map" }),
        }).then((r) => r.json()).catch(() => ({ stdout: "" })),
      ]);

      // Fetch state
      const stateRes = await fetch("/api/state").then((r) => r.json()).catch(() => ({}));
  ...
  const runImpact = async () => {
    ...
    try {
      const res = await fetch("/api/command/sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "impact", args: [description.trim()] }),
      });
      const data = await res.json();
      ...
      <div key={step} className="w-full max-w-[640px] stage-enter">
        ...
        {step === "describe" && (
          <div className="flex flex-col gap-6">
            ...
            <div
              className="rounded-2xl border border-border bg-bg-card p-5 flex flex-col gap-3"
