...
export function Chat() {
  ...
  // Watch for pending chat messages from Kadima prompt
  ...
  // Auto-submit after input was set from pending message
  useEffect(() => {
    if (pendingAutoSendRef.current && input) {
  ...
  /* ── Kadima action execution ── */
  async function runKadimaAction(sid: string, action: string, args?: Record<string, string>) {
    setSessionLoading(sid, true);
    appendLine(sid, { id: uid(), type: "status", text: `  ⚡ Kadima: ${action}` });
    ...
    try {
      ...
      switch (action) {
        ...
        case "deny":
          ...
          break;
        case "allocate":
          data = await fetch("/api/kadima/enqueue", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      appendLines(sid, output.split("\n").map((l: string) => ({
        id: uid(), type: "output" as const, text: `  ${l}`, dir: detectDir(l),
      })));
      appendLine(sid, { id: uid(), type: "status", text: "  ✓ Done" });
    } catch (err: any) {
      appendLine(sid, { id: uid(), type: "error", text: `  Error: ${err.message}` });
    }
    setSessionLoading(sid, false);
  }
  ...
  async function send() {
    ...
    // Three-way intent routing: Kadima → Ogu → Claude
    const intent = resolveIntent(trimmed);

    if (intent.route === "kadima") {
      await runKadimaAction(active.id, intent.kadimaAction!, intent.kadimaArgs);
      return;
    }

    if (intent.route === "ogu" && intent.oguCommand?.type === "command") {
      const { command, args, description } = intent.oguCommand.data;
  ...
  return (
    <div className="flex flex-1 gap-3">
      {/* -- Left: File tree -- */}
      <div className="w-[260px] rounded-2xl overflow-hidden bg-bg-card border border-border">
        <FileTreePanel />
      </div>

      {/* -- Center: Terminal -- */}
      <div className="flex flex-col flex-1 min-w-[400px] rounded-2xl overflow-hidden bg-bg-card border border-border">

        {/* Title bar */}
        <div className="flex items-center h-9 px-4 gap-2" style={{ borderBottom: "1px solid var(--border)" }}>
          <div style={{
          ...
          <span className="text-xs ml-1 text-text-muted">Ogu Studio</span>
          <Badge variant="outline">via Kadima</Badge>
          ...
          <span
            ...
            onMouseOver={(e) => { e.currentTarget.style.color = "var(--error)"; e.currentTarget.style.backgroundColor = "var(--color-error-soft)"; }}
        ...
        <div className="flex flex-1">
          ...
          >
            <div className="p-4" style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {(() => {
                ...
                return (
                  <div key={l.id} className="chat-line">
                    ...
                    <div style={{
                      ...
                      backgroundColor: l.type === "prompt" ? "var(--color-accent-soft)" : isInterrupted ? "var(--color-error-soft)" : undefined,
                    }}
                    >
                      <span style={{
                          flex: 1, fontSize: 13, lineHeight: "19px",
                          color: l.type === "meta" ? "var(--text-muted)" : colorMap[l.type],
                          direction: lineDir[idx],
                          textAlign: lineDir[idx] === "rtl" ? "right" : "left",
                          fontWeight: isToolBlock ? "300" : undefined,
                          ...(l.type === "meta" ? { fontSize: 11 } : isToolBlock ? { fontSize: 11 } : {}),
