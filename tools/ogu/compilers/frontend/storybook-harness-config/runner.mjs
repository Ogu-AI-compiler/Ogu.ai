...
const GATES = ['spec-valid','main-config','typescript-enabled','a11y-addon-installed','no-todos','contract-storybook-harness'];
...
export async function run({ dir: targetDir, projectRoot } = {}) {
  ...
  const artifact = { compiler: 'storybook-harness-config', pass: passed, timestamp: new Date().toISOString(), gates: results, attestation: createHash('sha256').update(JSON.stringify(results)).digest('hex') };
  if (targetDir) writeFileSync(join(targetDir, 'storybook-harness-artifact.json'), JSON.stringify(artifact, null, 2));
  return artifact;
}
