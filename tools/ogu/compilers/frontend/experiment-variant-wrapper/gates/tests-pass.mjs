...
export async function run({ dir, projectRoot }) {
  ...
  if (!testFiles.length) return { pass: false, code: 'EV008', message: 'No test file found' };
  ...
  try {
    ...
    return { pass: true, code: 'EV008', message: 'Tests passed' };
  } catch (e) {
    const summary = (e.stdout?.toString() || e.message).split('\n').filter(l => /FAIL|PASS|✓|✗|×|●/.test(l)).slice(0,10).join('\n');
    return { pass: false, code: 'EV008', message: 'Tests failed', detail: summary };
  }
}
