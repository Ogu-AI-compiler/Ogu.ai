...
export async function run({ dir }) {
  ...
  if (violations.length) return { pass: false, code: 'EV003', message: `Explicit 'any' in ${violations.length} location(s)`, detail: violations.slice(0,5).join('\n') };
  return { pass: true, code: 'EV003', message: 'No explicit any types' };
}
