...
const GATES = ['spec-valid','ts-valid','no-any','class-api','safe-fallback','structured-report','no-todos','tests-pass','contract-error-boundary'];
...
export async function run({ dir: targetDir, projectRoot } = {}) {
  ...
  const artifact = { compiler: 'error-boundary-wrapper', pass: passed, timestamp: new Date().toISOString(), gates: results, attestation: createHash('sha256').update(JSON.stringify(results)).digest('hex') };
  if (targetDir) writeFileSync(join(targetDir, 'error-boundary-artifact.json'), JSON.stringify(artifact, null, 2));
  return artifact;
}
