...
export async function run({ dir }) {
  ...
  if (violations.length) return { pass: false, code: 'EB003', message: `Explicit 'any' found in ${violations.length} location(s)`, detail: violations.slice(0, 5).join('\n') };
  return { pass: true, code: 'EB003', message: 'No explicit any types' };
}
