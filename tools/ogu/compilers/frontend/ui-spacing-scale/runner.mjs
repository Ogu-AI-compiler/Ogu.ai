/**
 * UI Spacing Scale Compiler — Runner
 */
...
const GATE_MODULES = {
  ...
  'base-multiples':  () => import('./gates/base-multiples.mjs'),
  'semantic-roles':  () => import('./gates/semantic-roles.mjs'),
  'no-duplicates':   () => import('./gates/no-duplicates.mjs'),
  'min-spacing':     () => import('./gates/min-spacing.mjs'),
  'no-todos':        () => import('./gates/no-todos.mjs'),
  'contract-spacing':() => import('./gates/contract-spacing.mjs'),
};

async function runGate(gateId, ctx) {
  const loader = GATE_MODULES[gateId];
  if (!loader) {
    return { id: gateId, pass: false, code: 'USS000', message: `Unknown gate: ${gateId}` };
  }
  try {
    ...
    return { id: gateId, pass: false, code: 'USS000', message: `Gate threw: ${err.message}`, detail: err.stack };
  }
}
...
export async function run({ dir, verbose = false } = {}) {
  ...
  if (existsSync(specPath)) {
    try {
      ...
      artifact.base_unit = spec.baseUnit || null;
      const roles = spec.semanticRoles ? Object.keys(spec.semanticRoles).length : 0;
      artifact.semantic_role_count = roles;
    } catch {}
  }

  if (overallPass) {
    writeFileSync(join(dir, 'spacing-artifact.json'), JSON.stringify(artifact, null, 2));
  }
