...
const GATES = ['spec-valid','vitest-config','jestdom-setup','coverage-config','no-todos','contract-test-harness'];
...
export async function run({ dir: targetDir, projectRoot } = {}) {
  ...
  const artifact = { compiler: 'testing-harness-config', pass: passed, timestamp: new Date().toISOString(), gates: results, attestation: createHash('sha256').update(JSON.stringify(results)).digest('hex') };
  if (targetDir) writeFileSync(join(targetDir, 'test-harness-artifact.json'), JSON.stringify(artifact, null, 2));
  return artifact;
}
