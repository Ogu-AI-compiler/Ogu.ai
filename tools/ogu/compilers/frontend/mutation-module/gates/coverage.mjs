...
export async function run({ dir, projectRoot }) {
  ...
  if (!testFiles.length) {
    return { pass: false, code: 'MU010', message: 'No test file found for coverage check' };
  }
  ...
  try {
    ...
    if (pct !== null && pct < THRESHOLD) {
      return { pass: false, code: 'MU010', message: `Coverage ${pct.toFixed(1)}% below ${THRESHOLD}% threshold` };
    }
    return { pass: true, code: 'MU010', message: pct ? `Coverage ${pct.toFixed(1)}%` : 'Coverage passed' };
  } catch (e) {
    return { pass: false, code: 'MU010', message: 'Coverage check failed', detail: e.stdout?.toString().slice(0, 300) };
  }
}
