...
const GATES = [
  { id: 'NC001', name: 'spec-valid',          phase: 1 },
  { id: 'NC002', name: 'ts-valid',            phase: 2 },
  { id: 'NC003', name: 'no-any',              phase: 2 },
  { id: 'NC004', name: 'nav-routes-valid',    phase: 3 },
  { id: 'NC005', name: 'active-state',        phase: 3 },
  { id: 'NC006', name: 'keyboard-accessible', phase: 3 },
  { id: 'NC007', name: 'no-todos',            phase: 4 },
  { id: 'NC008', name: 'tests-pass',          phase: 4 },
  { id: 'NC009', name: 'cross-routing',       phase: 5 },
  { id: 'NC010', name: 'contract-nav',        phase: 6 },
];
...
async function run() {
  ...
  console.log('\n┌─────────────────────────────────────────┐');
  console.log('│       Navigation Config Compiler        │');
  console.log('└─────────────────────────────────────────┘');
  ...
  const spec = JSON.parse(readFileSync(join(dir, 'nav-spec.json'), 'utf8'));
  const artifact = {
    compiler: 'navigation-config', version: '1.0.0', compiled_at: new Date().toISOString(),
    nav_items: (spec.items || []).map(i => ({ label: i.label, path: i.path })),
  ...
  writeFileSync(join(dir, 'nav-artifact.json'), JSON.stringify(artifact, null, 2));
  console.log(`\n  ✓ All gates passed\n  ✓ nav-artifact.json written\n  ✓ Attestation: ${artifact.attestation_hash.slice(0, 16)}...\n`);
}
