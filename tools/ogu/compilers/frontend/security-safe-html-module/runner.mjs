...
const GATES = ['spec-valid','ts-valid','no-any','no-raw-innerhtml','url-allowlist','xss-tests','no-todos','tests-pass','contract-safe-html'];
...
export async function run({ dir: targetDir, projectRoot } = {}) {
  ...
  const artifact = { compiler: 'security-safe-html-module', pass: passed, timestamp: new Date().toISOString(), gates: results, attestation: createHash('sha256').update(JSON.stringify(results)).digest('hex') };
  if (targetDir) writeFileSync(join(targetDir, 'safe-html-artifact.json'), JSON.stringify(artifact, null, 2));
  return artifact;
}
