#!/usr/bin/env node
/**
 * Animation Spec Compiler Runner
 * Compiles motion intent into Framer Motion variants with reduced-motion fallbacks.
 * Produces: animation-artifact.json
 */
...
const GATES = [
  { id: 'AN001', name: 'spec-valid',        phase: 1 },
  { id: 'AN002', name: 'easing-valid',      phase: 1 },
  { id: 'AN003', name: 'ts-valid',          phase: 2 },
  { id: 'AN004', name: 'no-any',            phase: 2 },
  { id: 'AN005', name: 'reduced-motion',    phase: 3 },
  { id: 'AN006', name: 'no-layout-thrash',  phase: 3 },
  { id: 'AN007', name: 'duration-bounds',   phase: 3 },
  { id: 'AN008', name: 'no-todos',          phase: 4 },
  { id: 'AN009', name: 'tests-pass',        phase: 4 },
  { id: 'AN010', name: 'cross-tokens',      phase: 5 },
  { id: 'AN011', name: 'contract-animation',phase: 6 },
];
...
async function run() {
  ...
  console.log('\n┌─────────────────────────────────────────┐');
  console.log('│        Animation Spec Compiler          │');
  console.log('└─────────────────────────────────────────┘');
  ...
  const artifact = {
    compiler: 'animation-spec', version: '1.0.0', compiled_at: new Date().toISOString(),
    name: spec.name, engine: spec.engine || 'framer-motion',
    variants: Object.keys(spec.variants),
    has_reduced_motion: !!spec.reducedMotion,
  ...
  writeFileSync(join(dir, 'animation-artifact.json'), JSON.stringify(artifact, null, 2));
  console.log(`\n  ✓ All gates passed\n  ✓ animation-artifact.json written\n  ✓ Attestation: ${artifact.attestation_hash.slice(0, 16)}...\n`);
}
