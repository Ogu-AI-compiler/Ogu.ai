...
export async function run({ dir }) {
  ...
  if (violations.length) return { pass: false, code: 'SW003', message: `Explicit 'any' in ${violations.length} location(s)`, detail: violations.slice(0,5).join('\n') };
  return { pass: true, code: 'SW003', message: 'No explicit any types' };
}
