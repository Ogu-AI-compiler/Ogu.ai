...
export async function run({ dir, projectRoot }) {
  ...
  if (!testFiles.length) return { pass: false, code: 'SW009', message: 'No test file found' };
  ...
  try {
    ...
    return { pass: true, code: 'SW009', message: 'Tests passed' };
  } catch (e) {
    const summary = (e.stdout?.toString() || e.message).split('\n').filter(l => /FAIL|PASS|✓|✗|×|●/.test(l)).slice(0,10).join('\n');
    return { pass: false, code: 'SW009', message: 'Tests failed', detail: summary };
  }
}
