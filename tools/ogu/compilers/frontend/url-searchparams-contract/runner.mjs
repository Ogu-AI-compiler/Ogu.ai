...
const GATES = ['spec-valid','ts-valid','no-any','total-parse','canonical-serialize','default-omission','no-todos','tests-pass','contract-searchparams'];
...
export async function run({ dir: targetDir, projectRoot } = {}) {
  ...
  const artifact = { compiler: 'url-searchparams-contract', pass: passed, timestamp: new Date().toISOString(), gates: results, attestation: createHash('sha256').update(JSON.stringify(results)).digest('hex') };
  if (targetDir) writeFileSync(join(targetDir, 'searchparams-artifact.json'), JSON.stringify(artifact, null, 2));
  return artifact;
}
