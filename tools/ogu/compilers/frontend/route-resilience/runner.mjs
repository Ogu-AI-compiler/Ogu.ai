...
const GATES = [
  { id: 'RR001', name: 'spec-valid',          phase: 1 },
  { id: 'RR002', name: 'ts-valid',            phase: 2 },
  { id: 'RR003', name: 'no-any',              phase: 2 },
  { id: 'RR004', name: 'error-boundary',      phase: 3 },
  { id: 'RR005', name: 'not-found-route',     phase: 3 },
  { id: 'RR006', name: 'suspense-fallback',   phase: 3 },
  { id: 'RR007', name: 'no-todos',            phase: 4 },
  { id: 'RR008', name: 'tests-pass',          phase: 4 },
  { id: 'RR009', name: 'cross-routing',       phase: 5 },
  { id: 'RR010', name: 'contract-resilience', phase: 6 },
];
...
async function run() {
  ...
  console.log('\n┌─────────────────────────────────────────┐');
  console.log('│        Route Resilience Compiler        │');
  console.log('└─────────────────────────────────────────┘');
  ...
  const spec = JSON.parse(readFileSync(join(dir, 'resilience-spec.json'), 'utf8'));
  const artifact = {
    compiler: 'route-resilience', version: '1.0.0', compiled_at: new Date().toISOString(),
    error_boundary: spec.errorBoundary,
    not_found_path: spec.notFoundPath,
    tier: 'frontend',
  ...
  writeFileSync(join(dir, 'resilience-artifact.json'), JSON.stringify(artifact, null, 2));
  console.log(`\n  ✓ All gates passed\n  ✓ resilience-artifact.json written\n  ✓ Attestation: ${artifact.attestation_hash.slice(0, 16)}...\n`);
}
