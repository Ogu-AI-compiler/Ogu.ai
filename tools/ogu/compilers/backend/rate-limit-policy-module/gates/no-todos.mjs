...
export async function run({ dir }) {
  ...
  if (v.length) return { pass: false, code: 'RL005', message: `${v.length} TODO/FIXME/HACK`, detail: v.slice(0, 10).join('\n') };
  return { pass: true, code: 'RL005', message: `No TODO/FIXME/HACK (${files.length} file(s))` };
}
