...
const GATES = [
  { id: 'EC001', name: 'spec-valid',                    phase: 1 },
  { id: 'EC002', name: 'cross-publisher',               phase: 1 },
  { id: 'EC003', name: 'envelope-validated-on-receipt', phase: 2 },
  { id: 'EC004', name: 'idempotency-persisted',         phase: 2 },
  { id: 'EC005', name: 'errors-classified',             phase: 3 },
  { id: 'EC006', name: 'channel-scoped',                phase: 3 },
  { id: 'EC007', name: 'no-todos',                      phase: 4 },
  { id: 'EC008', name: 'tests-pass',                    phase: 4 },
  { id: 'EC009', name: 'contract-event-consumer',       phase: 5 },
];
const PHASE_NAMES = { 1: 'Parse & Validate Spec', 2: 'Receipt Contract', 3: 'Error Handling', 4: 'Quality', 5: 'Attest' };
...
async function run() {
  ...
  console.log('\n┌─────────────────────────────────────────┐');
  console.log('│    Event Consumer Module Compiler       │');
  console.log('└─────────────────────────────────────────┘');
  ...
  const artifact = {
    compiler: 'event-consumer-module', version: '1.0.0', compiled_at: timestamp,
    ir_id: `EVENT_CONSUMER:${spec.eventType}`,
    eventType: spec.eventType, channels: spec.channels, handlerName: spec.handlerName,
  ...
  writeFileSync(join(dir, 'event-consumer-artifact.json'), JSON.stringify(artifact, null, 2));
  console.log(`\n  ✓ event-consumer-artifact.json written — IR: ${artifact.ir_id}\n`);
}
