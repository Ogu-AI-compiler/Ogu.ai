...
const GATES = [
  { id: 'GR001', name: 'spec-valid',                phase: 1 },
  { id: 'GR002', name: 'cross-schema',              phase: 1 },
  { id: 'GR003', name: 'no-direct-db',              phase: 2 },
  { id: 'GR004', name: 'n-plus-one-guarded',        phase: 2 },
  { id: 'GR005', name: 'authorization-checked',     phase: 3 },
  { id: 'GR006', name: 'errors-mapped-to-graphql',  phase: 4 },
  { id: 'GR007', name: 'no-todos',                  phase: 5 },
  { id: 'GR008', name: 'tests-pass',                phase: 5 },
  { id: 'GR009', name: 'contract-graphql-resolver', phase: 6 },
];
const PHASE_NAMES = { 1: 'Parse & Validate Spec', 2: 'Data Access', 3: 'Security', 4: 'Error Handling', 5: 'Quality', 6: 'Attest' };
...
async function run() {
  ...
  console.log('\n┌─────────────────────────────────────────┐');
  console.log('│   GraphQL Resolver Module Compiler      │');
  console.log('└─────────────────────────────────────────┘');
  ...
  const artifact = {
    compiler: 'graphql-resolver-module', version: '1.0.0', compiled_at: timestamp,
    ir_id: `GRAPHQL_RESOLVER:${spec.schemaName}`,
    schemaName: spec.schemaName, resolverCount: spec.resolvers.length,
  ...
  writeFileSync(join(dir, 'graphql-resolver-artifact.json'), JSON.stringify(artifact, null, 2));
  console.log(`\n  ✓ graphql-resolver-artifact.json written — IR: ${artifact.ir_id}\n`);
}
