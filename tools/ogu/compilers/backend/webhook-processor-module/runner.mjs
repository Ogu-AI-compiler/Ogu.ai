...
const GATES = [
  { id: 'WH001', name: 'spec-valid',                  phase: 1 },
  { id: 'WH002', name: 'signature-verified',          phase: 2 },
  { id: 'WH003', name: 'raw-body-preserved',          phase: 2 },
  { id: 'WH004', name: 'fast-ack',                    phase: 3 },
  { id: 'WH005', name: 'async-handoff',               phase: 3 },
  { id: 'WH006', name: 'no-todos',                    phase: 4 },
  { id: 'WH007', name: 'tests-pass',                  phase: 4 },
  { id: 'WH008', name: 'contract-webhook-processor',  phase: 5 },
];
const PHASE_NAMES = { 1: 'Parse & Validate Spec', 2: 'Security', 3: 'Ack Pattern', 4: 'Quality', 5: 'Attest' };
...
async function run() {
  ...
  console.log('\n┌─────────────────────────────────────────┐');
  console.log('│   Webhook Processor Module Compiler     │');
  console.log('└─────────────────────────────────────────┘');
  ...
  const artifact = {
    compiler: 'webhook-processor-module', version: '1.0.0', compiled_at: timestamp,
    ir_id: `WEBHOOK_PROCESSOR:${spec.provider}`,
    provider: spec.provider, signatureAlgorithm: spec.signatureAlgorithm,
    signatureHeader: spec.signatureHeader, secretConfigKey: spec.secretConfigKey,
  ...
  writeFileSync(join(dir, 'webhook-processor-artifact.json'), JSON.stringify(artifact, null, 2));
  console.log(`\n  ✓ webhook-processor-artifact.json written — IR: ${artifact.ir_id}\n`);
}
