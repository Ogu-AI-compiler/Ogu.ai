...
export async function run({ dir, projectRoot }) {
  ...
  if (!testFiles.length) {
    return { pass: false, code: 'AE008', message: 'No test file found. Query hooks require tests with renderHook.' };
  }
  ...
  try {
    ...
    return { pass: true, code: 'AE008', message: 'Tests passed' };
  } catch (e) {
    const out = e.stdout?.toString() || e.message;
    const summary = out.split('\n').filter(l => /FAIL|PASS|✓|✗|×|●/.test(l)).slice(0, 10).join('\n');
    return { pass: false, code: 'AE008', message: 'Tests failed', detail: summary };
  }
}
