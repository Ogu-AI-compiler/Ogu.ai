...
export async function run({ dir }) {
  ...
  if (v.length) return { pass: false, code: 'QA048', message: `${v.length} TODO/FIXME/HACK`, detail: v.slice(0, 10).join('\n') };
  return { pass: true, code: 'QA048', message: `No TODO/FIXME/HACK (${files.length} file(s))` };
}
