...
export class RunnerPool {
  ...
  async dispatch(task) {
    ...
    const runnersDir = join(this.root, '.ogu/runners');
    mkdirSync(runnersDir, { recursive: true });

    // Load OrgSpec for routing
    const orgSpecPath = join(this.root, '.ogu/OrgSpec.json');
  ...
  _markTaskCompleted(taskId) {
    const statePath = join(this.root, '.ogu/state/scheduler-state.json');
  ...
  _updateFeatureTask(featureSlug, taskId, status) {
    const featurePath = join(this.root, `.ogu/state/features/${featureSlug}.state.json`);
