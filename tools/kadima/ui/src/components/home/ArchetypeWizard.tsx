...
type Stage = "input" | "classifying" | "researching" | "classify" | "disambiguate" | "clarify" | "steps" | "launching";
...
interface ResearchReport {
  ...
  user_goals: string[];
  competitors: Array<{ name: string; url: string; positioning: string; strength: string; weakness: string }>;
  defensible_angle: { open_angle: string; rationale: string; confidence: number } | null;
  tech_feasibility: { build_complexity: string; key_libraries: string[]; known_pitfalls: string[] } | null;
  trend: { momentum: string; recent_events: string[] };
  assets: Array<{ type: string; url?: string; value?: string; source: string; validated: boolean }>;
  question_elimination_map: Record<string, string | string[]>;
  low_confidence_questions: string[];
  gaps: string[];
  confidence: { entity: number; competitors: number; assets: number; tech: number; trends: number };
  description: string;
}
...
type ClarifyStepKey = "foundation" | "focus" | "experience";
...
const CLARIFY_STEPS: Array<{ id: string; title: string; description: string; key: ClarifyStepKey }> = [
  { id: "clarify_foundation", title: "Foundation", description: "Language, typography, palette", key: "foundation" },
  { id: "clarify_focus", title: "Focus", description: "What this product covers", key: "focus" },
  { id: "clarify_experience", title: "Experience", description: "Layout, media, and feel", key: "experience" },
];
...
const CLARIFY_FOUNDATION_RE = /language|lang|rtl|ltr|font|typography|typeface|palette|color|brand/i;
const CLARIFY_FOCUS_RE = /type|category|categories|focus|topic|cuisine|product|audience|marketplace|listing|track|schedule|book|booking|service|industry|niche|coverage|cover/i;

function classifyClarifyQuestion(q: PersonalizedQuestion): ClarifyStepKey {
  const text = `${q.id || ""} ${q.prompt || ""}`.toLowerCase();
  if (CLARIFY_FOUNDATION_RE.test(text)) return "foundation";
  if (CLARIFY_FOCUS_RE.test(text)) return "focus";
  return "experience";
}

function splitClarifyQuestions(questions: PersonalizedQuestion[]) {
  const grouped: Record<ClarifyStepKey, PersonalizedQuestion[]> = {
    foundation: [],
    focus: [],
    experience: [],
  };
  for (const q of questions) {
    grouped[classifyClarifyQuestion(q)].push(q);
  }
  return grouped;
}
...
export function ArchetypeWizard({ mode: initialMode, onBack }: { mode: WizardMode; onBack: () => void }) {
  ...
  const [researchSteps, setResearchSteps] = useState<ResearchStep[]>([
    { id: "intent_inference", label: "Inferring intent", done: false },
    { id: "parallel_search",  label: "Searching sources", done: false },
    { id: "cross_validation", label: "Validating findings", done: false },
    { id: "asset_validation", label: "Validating assets", done: false },
    { id: "synthesis",        label: "Synthesizing report", done: false },
  ]);
  ...
  }, [stage, selectedArchetype?.id]);

  useEffect(() => {
    if (stage !== "clarify") return;
    const grouped = splitClarifyQuestions(clarifyQuestions || []);
    const stepKeys = CLARIFY_STEPS.map((s) => s.key);
    const hasStep = (idx: number) => idx === 0 || grouped[stepKeys[idx]]?.length > 0;
    setClarifyStep((prev) => {
      let idx = Math.min(prev, stepKeys.length - 1);
      if (hasStep(idx)) return idx;
      for (let i = idx + 1; i < stepKeys.length; i++) {
        if (hasStep(i)) return i;
      }
  ...
  const handleResearch = useCallback(async () => {
    ...
    // If we already have a report for this exact description, skip straight to classify
    if (researchReport && researchReport.description === text) {
      handleClassify();
      return;
    }
    ...
    setResearchSteps(steps => steps.map(s => ({ ...s, done: false })));
    setStage("researching");

    try {
      const res = await api.researchWizard(text, activeMode);
      if (!res.ok || !res.body) {
        setResearchError("Research stream failed");
        setStage("input");
        return;
      }
        ...
        for (const line of lines) {
          if (line.startsWith("event:")) {
            ...
            try {
              ...
              if (currentEvent === "research:progress" && payload.done) {
                setResearchSteps(prev => prev.map(s => s.id === payload.step_id ? { ...s, done: true } : s));
              } else if (currentEvent === "research:complete" && payload.report) {
                setResearchReport(payload.report);
                setStage("classifying");
                handleClassify();
              } else if (currentEvent === "research:error") {
                setResearchError(payload.error || "Research failed");
                setStage("input");
              }
  ...
  }, [description, activeMode, researchReport]);
  ...
  const generatePalette = useCallback(async () => {
    ...
    try {
      const result = await api.generatePalette({
        description: description.trim(),
        archetypeId: selectedArchetype.id,
        detailLevel: classifyResult?.detail_level,
        previousAnswers: answersRef.current,
      });
      const palettes = (result.palettes || []).filter((p: any) => Array.isArray(p.colors) && p.colors.length > 0);
      setPaletteOptions(palettes);
      if (palettes.length > 0) {
        setAnswers((prev) => prev.visual_palette ? prev : ({ ...prev, visual_palette: palettes[0] }));
      }
    } catch (err: any) {
      setPaletteError(err.message || "Palette generation failed");
    } finally {
      setPaletteLoading(false);
    }
  }, [selectedArchetype, description, classifyResult]);

  useEffect(() => {
    if (stage === "clarify" && selectedArchetype) {
      generatePalette();
    }
  }, [stage, selectedArchetype, generatePalette]);
  ...
  const handleSelectArchetype = useCallback(async (archetypeId: string) => {
    ...
    try {
      ...
      const elimination = researchReport?.question_elimination_map || {};
      const eliminated: Record<string, any> = {};

      const questions = (result.questions || []).filter((q: any) => {
        if (q?.skipped) return false;
        // Skip questions that research already answered
        if (elimination[q.id] !== undefined) {
          eliminated[q.id] = elimination[q.id];
          return false;
        }
        return true;
      });
      ...
      // Auto-apply defaults + research-eliminated answers
      const defaults: Record<string, any> = { ...eliminated };
      for (const q of questions) {
      ...
      // Always show clarify stage (even if only palette widget is shown)
      setStage("clarify");
    } catch {
          ...
          <div className="wizard-card rounded-[28px] p-5 flex flex-col gap-4">
            {expandedBrief ? (
              ...
              <textarea
                ...
                onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey && stage !== "classifying") { e.preventDefault(); researchEnabled ? handleResearch() : handleClassify(); } }}
            ...
            <div className="flex items-center justify-between">
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                {/* Research toggle */}
                <button
                  onClick={() => setResearchEnabled(r => !r)}
                  disabled={stage === "classifying"}
                  title={researchEnabled ? "Research enabled — will investigate before classifying" : "Enable research mode"}
                  style={{
                    display: "flex", alignItems: "center", gap: 5,
                    padding: "0 10px", height: 34, borderRadius: 8,
                    background: researchEnabled ? "rgba(99,241,157,0.08)" : "rgba(255,255,255,0.04)",
                    border: researchEnabled ? "1px solid rgba(99,241,157,0.3)" : "1px solid rgba(255,255,255,0.09)",
                    color: researchEnabled ? "rgba(99,241,157,0.8)" : "rgba(255,255,255,0.35)",
                ...
                <IconBtn
                  onClick={() => researchEnabled ? handleResearch() : handleClassify()}
      ...
      <div className="flex flex-col flex-1 items-center justify-center px-6">
        <div key="researching" className="w-full max-w-[880px] stage-enter">
          ...
          <div className="wizard-card rounded-[28px] p-6 flex flex-col gap-5">
            <div>
              <h3 className="text-lg font-semibold text-text">Researching your idea</h3>
              <p className="text-xs text-text-muted mt-1">Investigating competitors, market, and assets...</p>
            </div>

            <div className="flex flex-col gap-3">
              {researchSteps.map((step, i) => {
                const prevDone = researchSteps.slice(0, i).every(s => s.done);
                const isActive = !step.done && prevDone;
                return (
                  <div key={step.id} className="flex items-center gap-3">
                    {step.done ? (
                      <div style={{ width: 18, height: 18, borderRadius: "50%", flexShrink: 0,
                        background: "rgba(99,241,157,0.12)", border: "1px solid rgba(99,241,157,0.35)",
                        display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <svg width="9" height="9" viewBox="0 0 12 12" fill="none" stroke="rgba(99,241,157,0.9)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <span className="text-sm" style={{
                      color: step.done ? "rgba(255,255,255,0.65)" : isActive ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.28)",
                    }}>
                      {step.label}
                    </span>
                    {step.done && (
                      <span className="text-xs ml-auto" style={{ color: "rgba(99,241,157,0.5)" }}>done</span>
                    )}
                  </div>
                );
              })}
            ...
            {researchError && (
              <div className="text-sm text-red-400 bg-red-400/10 rounded-lg px-3 py-2 flex items-center gap-2">
                <span>{researchError}</span>
                <button onClick={() => setStage("input")} className="ml-auto underline cursor-pointer text-xs">Back</button>
              </div>
            )}
          </div>
        </div>
  ...
  if (stage === "clarify" && selectedArchetype) {
    ...
    const grouped = splitClarifyQuestions(questions);
    const stepKeys = CLARIFY_STEPS.map((s) => s.key);
    const stepIndex = Math.min(clarifyStep, stepKeys.length - 1);
    const stepKey = stepKeys[stepIndex];
    const stepQuestions = grouped[stepKey] || [];
    const selectedPalette = answers.visual_palette;

    const requiredAnsweredStep = stepQuestions.every((q) => {
    ...
    const requiredAnsweredAll = questions.every((q) => {
      if (!q.required) return true;
      const val = answers[q.id];
      if (val === undefined || val === null || val === "") return false;
      if (Array.isArray(val) && val.length === 0) return false;
    ...
    const hasStepContent = (idx: number) => idx === 0 || (grouped[stepKeys[idx]] || []).length > 0;
    const nextStepIndex = (() => {
      for (let i = stepIndex + 1; i < stepKeys.length; i++) {
        if (hasStepContent(i)) return i;
      }
    ...
    const isLastClarifyStep = stepIndex >= stepKeys.length - 1;
      ...
      <div className="flex flex-col flex-1 items-center justify-start px-6 py-10">
        <div key={stage} className="w-full max-w-[880px] stage-enter">
          ...
          <div className="wizard-card wizard-scroll rounded-[28px] p-6 flex flex-col gap-5">
            ...
            {researchReport && (
              <div style={{
                background: "rgba(99,241,157,0.05)", border: "1px solid rgba(99,241,157,0.18)",
                borderRadius: 8, padding: "7px 12px", display: "flex", alignItems: "center", gap: 8,
              }}>
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="rgba(99,241,157,0.7)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
                </svg>
                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.45)" }}>
                  Research found {researchReport.competitors.length} competitor{researchReport.competitors.length !== 1 ? "s" : ""}
                  {researchReport.defensible_angle ? ` · ${researchReport.defensible_angle.open_angle.slice(0, 55)}…` : ""}
                </span>
                {Object.keys(researchReport.question_elimination_map).length > 0 && (
                  <span style={{ fontSize: 10, marginLeft: "auto", color: "rgba(99,241,157,0.5)", whiteSpace: "nowrap" }}>
                    {Object.keys(researchReport.question_elimination_map).length} pre-answered
                  </span>
                )}
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <h3 className="text-lg font-semibold text-text">{CLARIFY_STEPS[stepIndex].title}</h3>
                <span className="text-xs text-text-muted">{CLARIFY_STEPS[stepIndex].description}</span>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                {paletteLoading ? (
                  <div className="flex items-center gap-3 py-2">
                    <span className="letter-shimmer text-sm font-medium">Generating palettes...</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                    {paletteOptions.map((p, idx) => {
                      ...
                      return (
                        <button
                          key={p.id || p.name || idx}
                          onClick={() => handleAnswer("visual_palette", p)}
            ...
            <div className="flex items-center justify-between pt-2">
              <button
                onClick={() => {
                  if (stepIndex > 0) {
                    setClarifyStep(prevStepIndex);
                  } else {
                    handleBackFromClarify();
                  }
                }}
                style={{
              ...
              {!isLastClarifyStep ? (
                <button
                  onClick={() => setClarifyStep(nextStepIndex)}
                  disabled={!requiredAnsweredStep}
                  style={{
                    display: "flex", alignItems: "center",
                    padding: "0 16px", height: 32, borderRadius: 8,
                    background: requiredAnsweredStep ? "rgba(99,241,157,0.12)" : "rgba(255,255,255,0.08)",
                    border: requiredAnsweredStep ? "1px solid rgba(99,241,157,0.45)" : "1px solid rgba(255,255,255,0.12)",
                    color: requiredAnsweredStep ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.5)",
                    fontSize: 13, fontWeight: 600, cursor: requiredAnsweredStep ? "pointer" : "not-allowed",
                    boxShadow: requiredAnsweredStep ? "0 0 16px rgba(99,241,157,0.18)" : "none",
                  }}
                >
                  Next
                </button>
                ...
                <button
                  ...
                  disabled={!requiredAnsweredAll}
                  style={{
                    display: "flex", alignItems: "center",
                    padding: "0 16px", height: 32, borderRadius: 8,
                    background: requiredAnsweredAll ? "rgba(99,241,157,0.12)" : "rgba(255,255,255,0.08)",
                    border: requiredAnsweredAll ? "1px solid rgba(99,241,157,0.45)" : "1px solid rgba(255,255,255,0.12)",
                    color: requiredAnsweredAll ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.5)",
                    fontSize: 13, fontWeight: 600, cursor: requiredAnsweredAll ? "pointer" : "not-allowed",
                    boxShadow: requiredAnsweredAll ? "0 0 16px rgba(99,241,157,0.18)" : "none",
                  }}
                >
                  Continue
                </button>
