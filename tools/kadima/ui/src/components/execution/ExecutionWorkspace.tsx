...
export function ExecutionWorkspace() {
  ...
  const handleResume = useCallback(async () => {
    ...
    addActivityLine("Resume requested", "pipeline");
    try {
      await api.resumeProject(activeProjectSlug);
      addActivityLine("Resume signal sent", "pipeline");
      const state = await api.getProjectState(activeProjectSlug).catch(() => null);
      if (state) setProjectUIState(state);
    } catch (err: any) {
      addActivityLine(`Resume failed: ${err?.message || "unknown error"}`, "error");
    } finally {
      setResumePending(false);
    }
  }, [activeProjectSlug, addActivityLine]);
              ...
              <>
                {executionStatus === "paused" ? (
                  <button
                    ...
                    className={`home-chip flex items-center gap-2 px-3 py-2 rounded-xl transition-all ${resumePending ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
                    style={{
