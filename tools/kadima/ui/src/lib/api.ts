...
export const api = {
  ...
  // Returns raw Response so caller can read SSE stream
  researchWizard: (description: string, mode: string, slug?: string) =>
    fetch(`${BASE}/wizard/research`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...getAuthHeaders() },
      body: JSON.stringify({ description, mode, slug }),
    }),

  generatePalette: (data: { description: string; archetypeId?: string; detailLevel?: string; previousAnswers?: Record<string, any> }) =>
    request<{ palettes: any[]; model: string; cost: number }>("/wizard/palette", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  dispatchProject: (slug: string) =>
    request<{ accepted: boolean; slug: string }>(`/project/${slug}/dispatch`, { method: "POST" }),
